...
print(df.sort_values(['Name', 'HP'], ascending=[1, 0])) #This will print name in ascending order (1=True) & (0=False) - HP in descending order
...
#OR DO THIS
col = ['#','Name','Type 1','Type 2','Total','HP','Attack','Defense','Sp. Atk','Sp. Def', 'Speed','Generation','Legendary']
...
#To text file
#df.to_csv('modified.txt', index=False, sep="\t") #Using tab as separator, you can also use commas
...
print(df.loc[df['Type 1'].str.contains('fire|grass',flags=re.I, regex=True)]) #Ignores capitalization
...
print(df.groupby(['Type 1']).mean().sort_values('Defense', ascending=False))#To see which type have the highest average in defense
...
#Set an option to see just n rows or columns (limit) everytime, even if you imported a new data set
